...
module.exports = {
  ...
  async messageRun(message, args) {
    ...
    const response = await reRR(message.guild, targetChannel[0], targetMessage, reaction, role);
  ...
  async interactionRun(interaction) {
    ...
    const response = await reRR(interaction.guild, targetChannel, messageId, reaction, role);
    await interaction.followUp(response);
  },
};

async function reRR(guild, channel, messageId, reaction, role) {
